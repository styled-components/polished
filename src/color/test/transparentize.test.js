...
describe('transparentize', () => {
  ...
  it('should reduce the opacity of rgb(255, 0, 255) by 0.1', () => {
    expect(transparentize(0.1, 'rgb(255, 0, 255)')).toEqual(
      'rgba(255,0,255,0.9)',
    )
  })

  it('should reduce the opacity of rgba(255, 0, 0, 1) by 0.1', () => {
    expect(transparentize(0.1, 'rgba(101, 100, 205, .7)')).toEqual(
      'rgba(101,100,205,0.6)',
    )
  })

  it('should reduce the opacity of rgba(255, 0, 0, .5) by 0.3', () => {
    expect(transparentize(0.3, 'rgba(255, 0, 0, .5)')).toEqual(
      'rgba(255,0,0,0.2)',
    )
  })

  it('should reduce the opacity of rgba(255, 0, 0, .5) by 0.5', () => {
    expect(transparentize(0.5, 'rgba(255, 0, 0, .5)')).toEqual(
      'rgba(255,0,0,0)',
    )
  })

  it('should reduce the opacity of hsl(0, 0%, 100%) by 0.2', () => {
    expect(transparentize(0.2, 'hsl(0, 0%, 100%)')).toEqual(
  ...
  it('should reduce the opacity of hsla(0, 0%, 100%, .8) by 0.5', () => {
    expect(transparentize(0.5, 'hsla(0, 0%, 100%, .8)')).toEqual(
      'rgba(255,255,255,0.3)',
    )
  })

  it('should reduce the opacity of hsla(0, 0.5%, 0.5%, .1) by 0.4', () => {
    expect(transparentize(0.4, 'hsla(0, 0.5%, 0.5%, 0.4)')).toEqual(
      'rgba(0,0,0,0)',
    )
  })

  it('should not reduce the opacity below 0', () => {
    expect(transparentize(0.5, 'rgba(255, 0, 0, .2)')).toEqual(
      'rgba(255,0,0,0)',
    )
  })
