...
describe('test', () => {
  ...
  it('should throw an error if a percentage greater than 1 is passed', () => {
    const error = new Error('Passed an incorrect argument to tint, please pass a percentage less than or equal to 1.')
  ...
  it('should throw an error if the arguments are passed in the wrong order', () => {
    const error = new Error('Passed an incorrect argument to tint, please pass a percentage less than or equal to 1.')
    // $FlowIgnoreNextLine since the coming is invalid code, flow complains
    expect(() => { tint('#00f', 0.2) }).toThrow(error)
  })

  it('should throw an error if a valid string is not passed as a color', () => {
    const error = new Error('Passed an incorrect argument to tint, please pass a string representation of a color.')
    // $FlowIgnoreNextLine since the coming is invalid code, flow complains
    expect(() => { tint(1, 2.0) }).toThrow(error)
  })
})
