...
describe('pxto', () => {
  ...
  describe('converter', () => {
    ...
    it('should throw an error if a non-pixel value is passed for the first arg', () => {
      expect(() => ({ height: em('10em') })).toThrow('Expected a string ending in "px" or a number passed as the first argument to em(), got "10em" instead.')
    })

    it('should throw an error if a non-pixel value is passed for the second arg', () => {
      expect(() => ({ height: em('10px', '16em') })).toThrow('Expected a string ending in "px" or a number passed as the second argument to em(), got "16em" instead.')
    })

    it('should throw an error if an invalid pixel value is passed', () => {
      expect(() => ({ height: em('apx') })).toThrow('Passed invalid pixel value ("apx") to em(), please pass a value like "12px" or 12.')
    })

    it('should throw an error if an invalid base value is passed', () => {
      expect(() => ({ height: em('16px', 'apx') })).toThrow('Passed invalid base value ("apx") to em(), please pass a value like "12px" or 12.')
    })
  })
})
